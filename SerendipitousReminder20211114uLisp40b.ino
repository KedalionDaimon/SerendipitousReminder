/* DERIVATIVE OF DAVID JOHNSON-DAVIES' ULISP FOR ARM,
   FOR ADAFRUIT METRO M4 GRAND CENTRAL,
   MODIFIED BY NINO IVANOV, 14TH NOVEMBER 2021. */
...
// knowedge-length may be 1300
// max-sentence-length may be 25
// assoc-length may be 300
// on a stronger machine, without waiting too long
...
"(defun initvars () (progn \
(defvar *knowledge-length* 700) \
(defvar *assoc-length* 100) \
(defvar *max-sentence-length* 20) \
(defvar *knowledge* (gen-sym *knowledge-length* '())) \
(defvar *assox* (gen-sym *assoc-length* '())) \
(defvar *inpt* '()) \
(defvar *topics* '()) \
(defvar *found-relations* '()) \
(defvar *found-associations* '()) \
(defvar *best-matched-sentences* '()) \
(defvar *secondary-associations* '()) \
(defvar *in-depth* '()) \
(defvar *sentences-per-topic* 3) \
(defvar *update* t) \
(defvar *cr-delay* 50) \
(defvar *contrast-threshold* 1))) "
"(defun ncr (n lis) \
  (loop \
...
"(defun dai (el lis) \
  (cond ((null lis) '()) \
        ((ek (car lis) el) (dai el (cdr lis))) \
        (t (cons (car lis) (dai el (cdr lis)))))) "
"(defun muq (lis) \
  (cond ((null lis) '()) \
        (t (cons (car lis) (muq (dai (car lis) (cdr lis))))))) "
"(defun any-member-in-common (lis1 lis2) \
  (loop \
    (cond ((null lis1) (return '())) \
          ((mmb (pop lis1) lis2) (return t))))) "
"(defun pbf (el lis col) \
  (loop \
    (cond ((null lis) (return (reverse col))) \
          ((ek el (car lis)) (return (reverse col)))) \
    (push (pop lis) col))) "
"(defun bef (el lis) (pbf el lis '())) "
"(defun proto-determine-topics (sentence knowledge limit) \
  (loop \
    (cond ((> limit (length sentence)) (return sentence)) \
          ((null knowledge) (return sentence))) \
    (setq sentence (append (bef (car knowledge) sentence) \
                            (cdr (mmb (pop knowledge) sentence)))))) "
"(defun pproto-determine-topics (sentence sentence-kno limit) \
  (let ((intermediate-result sentence)) \
  (loop \
    (cond ((null sentence-kno) (return intermediate-result)) \
          ((and (not (null intermediate-result)) \
                (<= (length intermediate-result) limit)) \
            (return intermediate-result))) \
    (setq intermediate-result (proto-determine-topics intermediate-result \
                                                      (pop sentence-kno) \
                                                      limit))))) "
"(defun in-second-list (lis1 lis2 fst rmn) \
  (cond ((null lis1) (append (reverse fst) (reverse rmn))) \
        ((mmb (car lis1) lis2) \
          (in-second-list (cdr lis1) lis2 (cons (car lis1) fst) rmn)) \
        (t \
          (in-second-list (cdr lis1) lis2 fst (cons (car lis1) rmn))))) "
"(defun mutually-matched (lis1 lis2) \
  (muq (append (in-second-list lis1 lis2 '() '()) lis2))) "
"(defun determine-topics (sentence knowledge) \
  (cond ((null sentence) '()) \
        ((null (cdr sentence)) sentence) \
        (t (let ((lim (/ (length sentence) 4))) \
           (let ((limit (cond ((> lim 5) 5) \
                              ((< lim 1) 1) \
                              (t lim)))) \
           (tkf \
             5 \
             (mutually-matched \
               (pproto-determine-topics (muq sentence) \
                                        knowledge \
                                        limit) \
               (pproto-determine-topics (muq sentence) \
                                        (ncr 300 knowledge) \
                                        limit)))))))) "
"(defun proto-seek-containing-sentences (word sentences col) \
  (loop \
    (cond ((= (length col) *sentences-per-topic*) (return col)) \
          ((null sentences) (return col)) \
          ((mmb word (car sentences)) (push (car sentences) col))) \
    (pop sentences))) "
"(defun seek-containing-sentences (word sentences) \
  (reverse (proto-seek-containing-sentences word sentences '()))) "
"(defun seek-all-sentences (list-of-words sentences) \
  (let ((result '())) \
  (loop \
    (cond ((null list-of-words) (return (reverse result)))) \
    (push (append \
            '(T-R-I-E-D) \
            (list (list (list (list (car list-of-words))))) \
            (apply 'append (mapcar (lambda (x) (list 'F-O-U-N-D x)) \
              (seek-containing-sentences (pop list-of-words) sentences)))) \
          result)))) "
"(defun how-many-in-common (lis1 lis2) \
  (let ((result 0) \
        (l1 (length lis1)) \
        (l2 (length lis2))) \
  (loop \
    (cond ((null lis1) (return (/ result (* 0.5 (+ 1 l1 l2))))) \
          ((mmb (pop lis1) lis2) (setq result (+ 1 result))))))) "
"(defun implant-in-candidates (el lis) \
  (cond ((null lis) '()) \
        ((> (car el) (caar lis)) (cons el (b-l lis))) \
        (t (cons (car lis) (implant-in-candidates el (cdr lis)))))) "
"(defun dal (lis1 lis2) \
  (cond ((or (null lis1) (null lis2)) lis2) \
        (t (dal (cdr lis1) (dai (car lis1) lis2))))) "
"(defun find-best-associations (words word-sets) \
  (let ((result '((0) (0) (0) (0) (0) (0) (0) (0)))) \
  (loop \
    (cond ((null word-sets) \
            (return (cons '(A-S-S-O-C-I-A-T-I-O-N-S) \
                      (tkf 10 \
                        (dai '() \
                          (dal words \
                            (muq \
                              (apply 'append \
                                     (mapcar 'cadr result)))))))))) \
    (setq result \
          (implant-in-candidates \
            (list (how-many-in-common words (car word-sets)) (pop word-sets)) \
            result))))) "
"(defun seek-explicit-topics (lis) \
  (let ((result '())) \
  (loop \
    (cond ((null lis) (return (muq (mapcar 'car (reverse result))))) \
          ((listp (car lis)) (push (car lis) result))) \
    (pop lis)))) "
"(defun unlist-words (lis) \
  (let ((unlisted '())) \
  (loop \
    (cond ((null lis) (return (reverse unlisted))) \
          ((listp (car lis)) (push (car (pop lis)) unlisted)) \
          (t (push (pop lis) unlisted)))))) "
"(defun save () (eval (quote (cons 'save-image ())))) "
"(defun reload () (eval (quote (cons 'load-image ())))) "
"(defun slowpp (lis) \
  (progn \
  (loop \
    (cond ((null lis) (return '()))) \
    ...
    (delay *cr-delay*)) \
  (terpri))) "
"(defun proto-mutually-found (lis1 lis2) \
  (cond ((null lis1) 0) \
        ((mmb (car lis1) lis2) \
          (+ 1 (proto-mutually-found (cdr lis1) lis2))) \
        (t (proto-mutually-found (cdr lis1) lis2)))) "
"(defun mutually-found (lis1 lis2) \
  (+ (proto-mutually-found lis1 lis2) (proto-mutually-found lis2 lis1))) "
"(defun topics-in-order (topics sentence) \
  (cond ((null sentence) '()) \
        ((mmb (car sentence) topics) \
          (cons (car sentence) \
                (topics-in-order topics (cdr sentence)))) \
        (t (topics-in-order topics (cdr sentence))))) "
"(defun weigh-topic-order (ordered-topics topics sentence) \
  (let ((cmp-topics (topics-in-order topics sentence))) \
       (- (mutually-found (mapcar 'list (b-l ordered-topics) \
                                        (cdr ordered-topics)) \
                          (mapcar 'list (b-l cmp-topics) \
                                        (cdr cmp-topics))) \
          (mutually-found (mapcar 'list (b-l ordered-topics) \
                                        (cdr ordered-topics)) \
                          (mapcar 'reverse \
                            (mapcar 'list (b-l cmp-topics) \
                                          (cdr cmp-topics))))))) "
"(defun show-topic-notification (ordered-topics topics sentence) \
  (let ((val (weigh-topic-order ordered-topics topics sentence))) \
    (cond ((> (- val *contrast-threshold*) (/ (length ordered-topics) 3)) \
            'F-O-U-N-D---L-I-K-E-N-E-S-S) \
          ((< (- val *contrast-threshold*) 0) \
            'F-O-U-N-D---C-O-N-T-R-A-S-T) \
          (t 'F-O-U-N-D---S-T-A-N-D-A-R-D)))) "
"(defun proto-find-all-unknowns (lis) \
  (cond ((null lis) '()) \
        ((and (eq (caar lis) 'T-R-I-E-D) (null (cddar lis))) \
          (cons (caaar (cadar lis)) \
                (proto-find-all-unknowns (cdr lis)))) \
        (t (proto-find-all-unknowns (cdr lis))))) "
"(defun find-all-unknowns (lis) \
  (let ((pfind (proto-find-all-unknowns lis))) \
    (cond ((null pfind) '(A-L-L---T-O-P-I-C-S---F-O-U-N-D)) \
          (t (cons '(U-N-K-N-O-W-N---T-O-P-I-C-S) pfind))))) "
"(defun proto-find-all-knowns (lis) \
  (cond ((null lis) '()) \
        ((and (eq (caar lis) 'T-R-I-E-D) (not (null (cddar lis)))) \
          (cons (car lis) \
                (proto-find-all-knowns (cdr lis)))) \
        (t (proto-find-all-knowns (cdr lis))))) "
"(defun find-all-knowns (lis) \
  (let ((pfind (proto-find-all-knowns lis))) \
    (cond ((null pfind) '(N-O---T-O-P-I-C-S---F-O-U-N-D)) \
          (t pfind)))) "
"(defun proto-determine-prefix (ordered-topics topics lis) \
  (cond ((null lis) '()) \
        ((eq (car lis) 'F-O-U-N-D) \
           (cons (show-topic-notification ordered-topics topics (cadr lis)) \
             (cons (cadr lis) \
               (proto-determine-prefix ordered-topics topics (cddr lis))))) \
        (t (cons (car lis) \
                 (proto-determine-prefix ordered-topics topics (cdr lis)))))) "
"(defun found-with-prefix (lis) \
  (let ((llis (find-all-knowns lis))) \
    (cond ((eq (car llis) 'N-O---T-O-P-I-C-S---F-O-U-N-D) llis) \
          (t (let ((ordered-topics (topics-in-order *topics* *inpt*))) \
               (mapcar (lambda (x) (proto-determine-prefix ordered-topics \
                                                           *topics* \
                                                           x)) \
                       llis)))))) "
"(defun most-hits (second-topics knowledge) \
  (cond ((or (null knowledge) (null second-topics)) '()) \
        (t \
  (let ((v1 0) (c1 '()) (v2 0) (c2 '()) (v3 0) (c3 '()) \
        (v4 0) (c4 '()) (v5 0) (c5 '()) (v6 0) (c6 '()) \
        (newvalu 0) (newcand '())) \
  (loop \
    (cond ((null knowledge) (return (append (list c1) (list c2) (list c3) \
                                            (list c4) (list c5) (list c6))))) \
    (setq newcand (pop knowledge)) \
    (cond ((null newcand) '()) \
          (t \
    (setq newvalu (how-many-in-common second-topics newcand)) \
    (cond ((< v6 newvalu) (progn (setq c6 newcand) (setq v6 newvalu)))) \
    (cond ((<= v5 newvalu) (progn (setq c6 c5)      (setq v6 v5) \
                                  (setq c5 newcand) (setq v5 newvalu)))) \
    (cond ((<= v4 newvalu) (progn (setq c5 c4)      (setq v5 v4) \
                                  (setq c4 newcand) (setq v4 newvalu)))) \
    (cond ((<= v3 newvalu) (progn (setq c4 c3)      (setq v4 v3) \
                                  (setq c3 newcand) (setq v3 newvalu)))) \
    (cond ((<= v2 newvalu) (progn (setq c3 c2)      (setq v3 v2) \
                                  (setq c2 newcand) (setq v2 newvalu)))) \
    (cond ((<= v1 newvalu) (progn (setq c2 c1)      (setq v2 v1) \
                                  (setq c1 newcand) \
                                  (setq v1 newvalu))))))))))) "
"(defun ai () \
(progn \
(initvars) \
(princ '(WELCOME TO THE (((SERENDIPITOUS REMINDER))) SYSTEM)) \
(terpri) \
(delay *cr-delay*) \
(princ '(THIS IS A SYMBOL BASED EXPERIMENTAL ARTIFICIAL INTELLIGENCE)) \
(terpri) \
(delay *cr-delay*) \
(princ '(INPUT WORD LISTS TO INTERACT SUCH AS (CYCLOPES FORGE LIGHTNINGS))) \
(terpri) \
(delay *cr-delay*) \
(princ '(THE SYSTEM WILL REPLY TO TOPICS AND OFFER ASSOCIATIONS WITH)) \
(terpri) \
(delay *cr-delay*) \
(princ '(PAST INTERACTIONS WHERE THESE TOPICS WERE NOTED AS IMPORTANT)) \
(terpri) \
(delay *cr-delay*) \
(princ '(IF YOU WISH A PURSUIT OF SPECIFIC TOPICS PLACE THEM IN PARENTHESES)) \
(terpri) \
(delay *cr-delay*) \
(princ '(EVERY INTERACTION UPDATES THE KNOWLEDGE UNLESS Q USED TO QUERY ONLY)) \
(terpri) \
(delay *cr-delay*) \
(princ '(SPECIAL COMMANDS)) \
(terpri) \
(delay *cr-delay*) \
(princ '((SAVE) SAVES THE PRESENT STATE OF THE SYSTEM)) \
(terpri) \
(delay *cr-delay*) \
(princ '((RELOAD) RELOADS THE PRESENT STATE OF THE SYSTEM IF ANY IS SAVED)) \
(terpri) \
(delay *cr-delay*) \
(princ '((FORGET) DELETES THE LATEST SENTENCE FROM KNOWLEDGE)) \
(terpri) \
(delay *cr-delay*) \
(princ '((REGEN) DESTROYS AND REINITIALISES ALL KNOWLEDGE)) \
(terpri) \
(delay *cr-delay*) \
(princ '((DEL SPECIFIC WORDS SEQUENCE) DESTROYS ANY SUCH SENTENCE WORD GROUP)) \
(terpri) \
(delay *cr-delay*) \
(princ '((ERA SPECIFIC WORDS SEQUENCE) DESTROYS ANY SUCH ASSOCIATIONS GROUP)) \
(terpri) \
(delay *cr-delay*) \
(princ '((Q SPECIFIC WORDS SEQUENCE) ONLY QUERIES WITHOUT CHANGING KNOWLEDGE)) \
(terpri) \
(delay *cr-delay*) \
(princ '((N POSITIVE-INTEGER) UPDATES THE SENTENCES PER TOPIC TO BE GIVEN)) \
(terpri) \
(delay *cr-delay*) \
(princ '(ANY OF (QUIT END EXIT STOP SHUTUP SYSTEM) WILL RETURN YOU TO LISP)) \
(terpri) \
(delay *cr-delay*) \
(princ '(ENJOY YOUR INTERACTION)) \
(terpri) \
(loop \
(terpri) (princ '>) \
(setq *inpt* (tkl *max-sentence-length* (read))) \
(cond ((and (or (eq (car *inpt*) 'QUIT) \
                (eq (car *inpt*) 'END) \
                (eq (car *inpt*) 'EXIT) \
                (eq (car *inpt*) 'STOP) \
                (eq (car *inpt*) 'SHUTUP) \
                (eq (car *inpt*) 'SYSTEM)) \
            (null (cdr *inpt*))) (return '())) \
      ((null *inpt*) '()) \
      ((eq (car *inpt*) 'DEL) \
        (setq *knowledge* \
              (tkf *knowledge-length* \
                   (append (dai (cdr *inpt*) *knowledge*) '(()))))) \
      ((eq (car *inpt*) 'ERA) \
        (setq *assox* \
              (tkf *assoc-length* \
                   (append (dai (cdr *inpt*) *assox*) '(()))))) \
      ((eq (car *inpt*) 'N) (setq *sentences-per-topic* (cadr *inpt*))) \
      ((and (eq (car *inpt*) 'REGEN) (null (cdr *inpt*))) (progn \
         (setq *knowledge* (gen-sym *knowledge-length* '())) \
         (princ '(KNOWLEDGE RESET AND EMPTY)) (terpri))) \
      ((and (eq (car *inpt*) 'FORGET) (null (cdr *inpt*))) (progn \
         (setq *knowledge* (append (cdr *knowledge*) '(()))) \
         (princ '(LATEST SENTENCE FORGOTTEN)) (terpri))) \
      ((and (eq (car *inpt*) 'SAVE) (null (cdr *inpt*))) (progn \
         (save) (princ '(SYSTEM STATUS SAVED)) (terpri))) \
      ((and (eq (car *inpt*) 'RELOAD) (null (cdr *inpt*))) (progn \
         (reload) (princ '(SYSTEM STATUS RELOADED)) (terpri))) \
      (t (progn \
(cond ((eq (car *inpt*) 'Q) (progn (setq *update* '()) \
                                   (setq *inpt* (cdr *inpt*))))) \
(setq *topics* (seek-explicit-topics *inpt*)) \
(cond ((null *topics*) (setq *topics* (determine-topics *inpt* *knowledge*)))) \
(setq *inpt* (unlist-words *inpt*)) \
(setq *found-relations* (seek-all-sentences *topics* *knowledge*)) \
(setq *found-associations* (find-best-associations *topics* *assox*)) \
(cond ((not (null *in-depth*)) (progn \
  (setq *best-matched-sentences* (cons '(B-E-S-T---M-A-T-C-H-E-S) \
    (most-hits (append *topics* *found-associations*) *knowledge*))) \
  (setq *secondary-associations* \
        (cons '(S-E-C-O-N-D-A-R-Y---A-S-S-O-C-I-A-T-I-O-N-S) (cdr \
          (find-best-associations (append *topics* (cdr *found-associations*)) \
                                  *assox*))))))) \
(cond (*update* (progn (setq *knowledge* (cons *inpt* (b-l *knowledge*))) \
                       (setq *assox* (cons *topics* (b-l *assox*))))) \
      (t (setq *update* t))) \
(slowpp (found-with-prefix *found-relations*)) \
(delay *cr-delay*) \
(princ (find-all-unknowns *found-relations*)) \
(terpri) (terpri) \
(delay *cr-delay*) \
(princ *found-associations*) \
(terpri) (terpri) \
(delay *cr-delay*) \
(cond ((not (null *in-depth*)) (progn \
  (princ *best-matched-sentences*) \
  (terpri) (terpri) \
  (princ *secondary-associations*) \
  (delay *cr-delay*) \
  (terpri) (terpri))))))))))"
" (defun code () (progn \
(defvar *coded-message* '()) \
(defvar *alphabets* (quote ( \
(! R Z B _ X O W 9 5 Y ? G T 0 K 1 C 3 6 P U & D 7 L M * I E A J H 8 % 4 2 V F S Q N +) \
(N Q 9 O 8 J 7 E U L 0 X 6 R Z W M % 3 F K 4 H 5 _ G & V B 2 * T I Y A + P C ? ! D S 1) \
(% U N ? E F 1 Y Q * T & S + _ Z 7 O P C W 5 L 4 ! V G R 6 K X A 0 2 H J 9 3 I B 8 M D) \
(C L U N 8 _ Y J 5 1 H Z X P Q % 7 ! T F + R 3 A O 9 4 ? S & 2 I 0 G B M E W * 6 D V K) \
(M 5 X G % 3 1 0 R _ H * O 7 ? Z ! W Q S A B + K 8 C 2 J 9 T U N I 6 F 4 L E V D Y & P) \
(E 9 M 6 F T 4 O 2 G U K P B 8 & 0 R Z L ? Y C _ ! D % J N V I Q 1 3 7 S A H + W 5 X *) \
(Y % 2 S U H R 7 6 Q Z I ! 8 G 3 B F T 4 9 K N 0 5 E P O M ? * W X 1 _ J & D C V A + L) \
(5 1 W ! N T * Z 2 + E 9 B R 6 % 4 F X 7 ? I O P M S 3 & D 8 U V Y G _ K L J H A Q 0 C) \
(7 * 2 L _ 1 I 0 6 V Q B + C D R Z M W E 9 5 S Y K 3 4 ? T ! N 8 G F % H J & X A O P U) \
(B ! 3 Q 1 % 2 G U Z * 8 R 6 & 4 Y S 5 E J 7 V M _ L 9 C T A F D P H X O N K 0 ? W + I) \
(9 7 J U 5 H O P Q W V B 3 6 E & F Y Z 0 ! 1 L _ K G R N 2 % X C T I 8 4 * S D + A ? M) \
(8 L R 3 V 7 6 Y Q 4 5 ? S 9 _ P 1 0 * U 2 E ! T Z O C A W M & + N D X J K B F H % I G) \
(? % T U X M L 0 _ P W 3 * B E Z J K D 8 ! 6 Y N + S & C A Q R 2 I O H F 4 G 1 9 5 7 V) \
(P 6 9 X 2 3 + S 5 0 1 8 ? D * O N _ L M Q U E % R Z I F V J Y B ! C & G 7 4 W A K H T) \
(S Z C 8 0 X I 6 4 5 7 & ! Y U O 9 D Q ? B R T A P J K L 1 3 _ V M N H 2 % + G F E * W) \
(% 2 U Z K L Q 6 D P * 1 7 Y X B + V M ? 9 0 8 N 5 4 J G _ W O S I R & ! C F E A H T 3) \
(Z 7 X E W Q 0 * J V 1 R T U % I O C ! S ? A 2 4 + 5 8 6 & N B G K Y P L M 9 3 _ H D F) \
(6 R 4 9 F ! L 2 U 3 8 * S 1 0 M B H Y ? 5 N % P W + Q T X G J Z K C 7 V _ A I O & D E) \
(N K P 3 E 1 8 9 J C V 5 G Y U _ O 7 S 2 Z 6 ? D ! W 4 0 * X + R Q % I B L M A F T H &) \
(& P % R B 9 + 1 U 2 5 _ K * I L V 8 Q C M N 7 D X E Z T A H F 6 O 4 S Y ? G ! J W 3 0) \
(4 H U W 1 S 9 8 3 7 K N L A E 6 T Z V B R ? D ! P F 2 5 X Y Q 0 & M + O I % * J C G _) \
(J 0 ? 6 _ * + 4 I 8 5 D P X C K H & 7 3 2 F % T Z 1 9 W Q L N O E ! Y A R S G V B U M) \
(W P Q _ ? S B V N + E 8 M 5 & I 6 K L * 2 7 D A 3 Z 4 U 1 G T C X O Y R J ! H 0 % 9 F) \
(E 6 K J X B T 2 + 4 5 ! Q R L P 0 9 S N 3 7 * % I U F _ H G Z & D V C 8 1 Y A M ? W O) \
(3 Y ? M E _ P * 9 1 S R T C L % 5 4 U + F B N & 0 ! I Q X G A O W H V K J 6 2 8 D Z 7) \
(* _ H P & K Z T L M N W J U R Y 4 S ! G E 8 6 + 0 2 1 D 3 X % C 5 7 F V ? B A Q O 9 I) \
(I Z L ! O S 1 5 8 G 0 J K U B P _ 9 Y % 2 7 V W & 3 ? + * H X C Q R E N M T F D A 4 6) \
(S + 4 6 E 9 8 7 U V G F 2 J _ B H K % I Q W 1 L Z R D X N P 5 T C ! M & Y A O 0 * ? 3) \
(& X I Y S ! G M K U F 6 1 5 + H Q C 0 L E 8 * 9 D _ J N W 7 V ? O P B R T A Z 4 3 % 2) \
(G 2 8 6 ? 3 0 % Q X 7 S _ O * T E P R L 5 N K 1 C U J Y A Z + I 9 4 ! W & V B D F H M) \
(6 % Q 2 B G I Z W ? 5 7 X H S N & M ! J K R L Y T E 1 U D F + A 0 3 4 8 C * P _ O V 9) \
))) \
(defvar *alphalength* (length (car *alphabets*))) \
(defvar *alphalength2* (* 2 *alphalength*)) \
(defvar *alphabet* (car *alphabets*)) \
(defvar *alphabet2* (append (car *alphabets*) (car *alphabets*))))) "
"(defun nth-cdr (n lis) \
  (cond ((null lis) lis) ((= 0 n) lis) (t (nth-cdr (- n 1) (cdr lis))))) "
"(defun setmsg () (setq *coded-message* (read))) "
"(defun no-negative-shift (shift-count) \
  (cond ((> 0 shift-count) (+ *alphalength* shift-count)) (t shift-count))) "
"(defun encode-letter (clear-letter key-letter alphabet alphabet2) \
  (car \
    (nth-cdr \
      (+ (- *alphalength* (length (member clear-letter alphabet))) \
         (- *alphalength* (length (member key-letter alphabet)))) \
    alphabet2))) "
"(defun decode-letter (coded-letter key-letter alphabet alphabet2) \
  (car \
    (nth-cdr (no-negative-shift \
      (- (- *alphalength* (length (member coded-letter alphabet))) \
         (- *alphalength* (length (member key-letter alphabet))))) \
    alphabet2))) "
"(defun encode-sequence (key-list clear-list alphabets) \
  (let ((result '())) \
    (loop \
      (cond ((or (null key-list) (null clear-list)) \
            (return (reverse result)))) \
      (push (encode-letter (pop clear-list) (pop key-list) \
                           (car alphabets) (append (car alphabets) \
                                                   (car alphabets))) \
            result) \
      (setq alphabets (append (cdr alphabets) (list (car alphabets))))))) "
"(defun decode-sequence (key-list clear-list alphabets) \
  (let ((result '())) \
    (loop \
      (cond ((or (null key-list) (null clear-list)) \
            (return (reverse result)))) \
      (push (decode-letter (pop clear-list) (pop key-list) \
                           (car alphabets) (append (car alphabets) \
                                                   (car alphabets))) \
            result) \
      (setq alphabets (append (cdr alphabets) (list (car alphabets))))))) "
"(defun take-first-list (measure list-to-cut) \
  (let ((result '())) \
    (loop \
      (cond ((or (null list-to-cut) (null measure)) \
             (return (reverse result)))) \
      (pop measure) \
      (push (pop list-to-cut) result)))) "
"(defun take-first-n (n list-to-cut) \
  (let ((result '())) \
    (loop \
      (cond ((or (null list-to-cut) (zerop n)) \
             (return (reverse result)))) \
      (decf n) \
      (push (pop list-to-cut) result)))) "
"(defun scramble-alphabets (n alphabets) \
  (cond ((> n (length *alphabets*)) \
         (scramble-alphabets (- n (length alphabets)) alphabets)) \
        (t (append (nth-cdr n alphabets) (take-first-n n alphabets))))) "
"(defun rotate-forward (n lis) \
  (loop \
  (cond ((zerop n) (return lis))) \
  (decf n) \
  (setq lis (append (cdr lis) (list (car lis)))))) "
"(defun rotate-back (n lis) \
  (loop \
  (cond ((zerop n) (return lis))) \
  (decf n) \
  (setq lis \
        (append (list (car (reverse lis))) (reverse (cdr (reverse lis))))))) "
"(defun stream-encode (password-list clear-text-list) \
  (let ((result '()) \
        (original-password password-list) \
        (alphabets *alphabets*) \
        (alphabet *alphabet*) \
        (alphabet2 (append *alphabet* *alphabet*)) \
        (pos 1)) \
    (loop \
      (setq pos (length (member (car password-list) alphabet))) \
      (setq alphabets (scramble-alphabets pos alphabets)) \
      (setq alphabet (car alphabets)) \
      (setq alphabet2 (append alphabet alphabet)) \
      (setq password-list \
           (rotate-forward \
             (+ 5 pos) \
             password-list)) \
      (setq original-password \
            (rotate-back  \
              (+ 7 (length (member (car (reverse original-password)) \
                                   alphabet)) \
                   (length (member (car (reverse password-list)) \
                                   alphabet))) \
              original-password)) \
      (cond ((null clear-text-list) (return result))) \
      (setq password-list \
           (encode-sequence password-list \
                            (take-first-list password-list clear-text-list) \
                            alphabets)) \
      (setq password-list (encode-sequence original-password password-list \
                          alphabets)) \
      (setq clear-text-list (nth-cdr (length password-list) clear-text-list)) \
      (setq result (append result password-list))))) "
"(defun stream-decode (password-list coded-text-list) \
  (let ((result '()) \
        (original-password password-list) \
        (alphabets *alphabets*) \
        (alphabet *alphabet*) \
            ...
            (rotate-back  \
              (+ 7 (length (member (car (reverse original-password)) \
                   ...
                   (length (member (car (reverse password-list)) \
                                   alphabet))) \
              original-password)) \
      (cond ((null coded-text-list) (return result))) \
      (setq password-list \
           (decode-sequence password-list \
                            (take-first-list password-list coded-text-list) \
                            alphabets)) \
      (setq result \
            (append result (decode-sequence original-password password-list \
                            alphabets))) \
      (setq password-list (take-first-list password-list coded-text-list)) \
      (setq coded-text-list \
            (nth-cdr (length password-list) coded-text-list))))) "
"(defun encode () \
  (let ((msg '())) \
    (progn \
    (princ '(I N P U T _ M E S S A G E _ T O _ E N C O D E *)) \
    (terpri) \
    (setq msg (read)) \
    (princ '(I N P U T _ P A S S W O R D *)) \
    (terpri) \
    (setq *coded-message* (stream-encode (read) msg)) \
    (setq msg '()) \
    (princ *coded-message*) \
    (terpri) \
    (princ '(E N C O D I N G _ F I N I S H E D *)) \
    (terpri)))) "
"(defun decode () \
  (progn \
  (princ *coded-message*) \
  (terpri) \
  (princ '(I N P U T _ P A S S W O R D _ T O _ D E C O D E _ M E S S A G E *)) \
  (terpri) \
  (princ (stream-decode (read) *coded-message*)) \
  (terpri) \
  (princ '(D E C O D I N G _ F I N I S H E D *)) \
  (terpri))) "
"(defun clear () (progn \
(makunbound 'PROTO-GEN-SYM) \
(makunbound 'GEN-SYM) \
(makunbound 'INITVARS) \
(makunbound 'NCR) \
(makunbound 'PROTO-TKF) \
(makunbound 'TKF) \
(makunbound 'TKL) \
(makunbound 'EK) \
(makunbound 'PBL) \
...
(makunbound 'DAI) \
(makunbound 'MUQ) \
(makunbound 'ANY-MEMBER-IN-COMMON) \
(makunbound 'PBF) \
(makunbound 'BEF) \
(makunbound 'PROTO-DETERMINE-TOPICS) \
(makunbound 'PPROTO-DETERMINE-TOPICS) \
(makunbound 'IN-SECOND-LIST) \
(makunbound 'MUTUALLY-MATCHED) \
(makunbound 'DETERMINE-TOPICS) \
(makunbound 'PROTO-SEEK-CONTAINING-SENTENCES) \
(makunbound 'SEEK-CONTAINING-SENTENCES) \
(makunbound 'SEEK-ALL-SENTENCES) \
(makunbound 'HOW-MANY-IN-COMMON) \
(makunbound 'IMPLANT-IN-CANDIDATES) \
(makunbound 'DAL) \
(makunbound 'FIND-BEST-ASSOCIATIONS) \
(makunbound 'SEEK-EXPLICIT-TOPICS) \
(makunbound 'UNLIST-WORDS) \
(makunbound 'SAVE) \
(makunbound 'RELOAD) \
(makunbound 'SLOWPP) \
(makunbound 'PROTO-MUTUALLY-FOUND) \
(makunbound 'MUTUALLY-FOUND) \
(makunbound 'TOPICS-IN-ORDER) \
(makunbound 'WEIGH-TOPIC-ORDER) \
(makunbound 'SHOW-TOPIC-NOTIFICATION) \
(makunbound 'PROTO-FIND-ALL-UNKNOWNS) \
(makunbound 'FIND-ALL-UNKNOWNS) \
(makunbound 'PROTO-FIND-ALL-KNOWNS) \
(makunbound 'FIND-ALL-KNOWNS) \
(makunbound 'PROTO-DETERMINE-PREFIX) \
(makunbound 'FOUND-WITH-PREFIX) \
(makunbound 'MOST-HITS) \
(makunbound 'RUN) \
(makunbound 'CODE) \
(makunbound 'SETMSG) \
(makunbound 'NTH-CDR) \
(makunbound 'NO-NEGATIVE-SHIFT) \
(makunbound 'ENCODE-LETTER) \
(makunbound 'DECODE-LETTER) \
(makunbound 'ENCODE-SEQUENCE) \
(makunbound 'DECODE-SEQUENCE) \
(makunbound 'TAKE-FIRST-LIST) \
(makunbound 'TAKE-FIRST-N) \
(makunbound 'SCRAMBLE-ALPHABETS) \
(makunbound 'ROTATE-FORWARD) \
(makunbound 'ROTATE-BACK) \
(makunbound 'STREAM-ENCODE) \
(makunbound 'STREAM-DECODE) \
(makunbound 'ENCODE) \
(makunbound 'DECODE))) "
;
...
void testescape () {
  if (SRL.read() == '~') error2(NIL, PSTR("escape!"));
}
...
object *eval (object *form, object *env) {
  ...
  EVAL:
  // Enough space?
  // SRL.println((uint32_t)sp - (uint32_t)&ENDSTACK); // Find best STACKDIFF value
...
void pserial (char c) {
  if ((c > 96) && (c < 123)) {c = c - 32;} // UPPERCASE TERMINAL
  LastPrint = c;
  if (c == '\n') SRL.write('\r');
  SRL.write(c);
  if (c == '\n') {delay(DLY * 10);}
...
void esc (int p, char c) {
  SRL.write('\e'); SRL.write('[');
  SRL.write((char)('0'+ p/100));
  SRL.write((char)('0'+ (p/10) % 10));
  SRL.write((char)('0'+ p % 10));
  SRL.write(c);
}

void hilight (char c) {
  SRL.write('\e'); SRL.write('['); SRL.write(c); SRL.write('m');
}
...
void processkey (char c) {
  ...
  if (c == 8 || c == 0x7f) {     // Backspace key
    if (WritePtr > 0) {
      WritePtr--;
      SRL.write(8); SRL.write(' '); SRL.write(8);
...
#else
  unsigned long start = millis();
  while (!SRL.available()) if (millis() - start > 1000) clrflag(NOECHO);
...
void loop () {
  ...
  delay(100); while (SRL.available()) SRL.read();
